...
function BuildCard({
  ...
  /* 0 = collapsed; 1–5 = how many layers have been revealed so far. */
  ...
  return (
    <article className={`build${primary ? " build--primary" : ""}`}>
      <div className="build__top">
        <span className="build__rank">
          {primary ? "Primary recommendation" : `Also worth scoping · ${rank} of 3`}
        </span>
      </div>
      ...
      >
        {step > 0 ? "Hide why" : "View why"}
        <ChevronDown className={step > 0 ? "is-open" : ""} aria-hidden />
      </button>

      {step > 0 && (
        <div className="build__why" id={panelId}>
 ...
 * recommendation, at most two alternates, each explainable on demand.
 */
export function RecommendedBuilds({ projectId }: { projectId: string }) {
  ...
  return (
    <section className="builds" aria-labelledby="builds-heading">
      ...
      <BuildCard build={primary} rank={1} projectId={projectId} />

      <div className="builds__secondary">
        {secondary.map((b, i) => (
          <BuildCard key={b.id} build={b} rank={i + 2} projectId={projectId} />
        ))}
      </div>
    </section>
  );
